...
static struct CassFdwOption valid_options[] = {
    ...
    { "querable_columns", ForeignTableRelationId},
    /* Sentinel */
    { NULL, InvalidOid}
};
...
static char* processWhereClause(Expr *expr, RelOptInfo *baserel, PlannerInfo *root);
        ...
        if (expr->type == T_OpExpr) {
            ...
            if (!(leftargtype == INTERVALOID && rightargtype == INTERVALOID)) {
                ...
                if (strcmp(opername, "=") == 0) {
                    ...
                    leftvalue = processWhereClause(left, baserel, root);

                    right = (Expr *) lsecond(oper->args);
                    rightvalue = processWhereClause(right, baserel, root);
